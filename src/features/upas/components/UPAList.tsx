    ...
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      <main className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Unidades de Saúde (UPAs)</h2>
          <button
            onClick={onAdd}
          ...
          <div className="space-y-3">
            {filtered.map((upa, index) => (
              <motion.div
                ...
                onClick={() => onSelect(String(upa.id))}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${upa.ativa ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400'}`}>
                    <Building2 size={24} />
                  </div>
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
                      ...
                      <span className="text-[10px] text-slate-400 flex items-center gap-1 uppercase font-bold tracking-wider">
                        <Users size={10} />{upa.total_medicos} Médicos
                      </span>
                      <span className="text-[10px] text-slate-400 flex items-center gap-1 uppercase font-bold tracking-wider">
                        <Stethoscope size={10} />{upa.em_atendimento} em atendimento
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className={`hidden sm:block px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${upa.ativa ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
